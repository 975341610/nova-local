...
import {
  ...
  cylinderMainPath,
  shapePath,
} from '../../../lib/whiteboard/shapeGeometry'
...
export function FreehandNodeView({ node, updateAttributes, selected }: Props) {
  ...
  // v0.21.10 · 修复: noopener 会让 window.open 返回 null, document.write 落不进去.
  //            改为: 生成 Blob URL 直接 open, 并在空画板时给 exportSvg 一个兜底 viewBox.
  const onOpenPreview = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation()
      try {
        const isEmptyPreview =
          data.nodes.length === 0 && data.edges.length === 0 && data.strokes.length === 0
        // 空画板时用一个占位 SVG, 否则 exportSvg() 已会给出 bbox 兜底
        const svg = isEmptyPreview
          ? `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 440" width="720" height="440"><rect width="720" height="440" fill="white"/><text x="360" y="220" text-anchor="middle" font-size="20" fill="#94a3b8" font-family="system-ui, sans-serif">空白画板 · 双击可编辑</text></svg>`
          : exportSvg(data)
        ...
        const url = URL.createObjectURL(blob)
        // 注意: 移除 noopener, 否则某些 Chromium/Electron 组合会返回 null
        const w = window.open(url, '_blank', 'width=960,height=640')
        // 兜底: 若被浏览器拦截, 则在当前标签新开一个 tab
        if (!w) {
          const a = document.createElement('a')
          a.href = url
          a.target = '_blank'
              ...
              return (
                <g key={e.id} style={{ color }}>
                  <path
                    ...
                    markerEnd={endStyle === 'none' ? undefined : 'url(#wb-disp-arrow)'}
                  />
                  {e.label && (
                    <g>
                      <rect
      ...
      case 'table': {
        ...
        return (
          <foreignObject x={n.x} y={n.y} width={n.w} height={n.h}>
            <div
              // @ts-expect-error xmlns required for foreignObject XHTML
