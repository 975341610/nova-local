...
export function createWhiteboardStore(initial: WhiteboardData): WhiteboardStore {
  ...
  return {
    ...
    removeSelected() {
      ...
      commit((d) => {
        d.nodes = d.nodes.filter((n) => !(ids.has(n.id) && !n.locked))
    ...
    groupSelected() {
      ...
      commit((d) => {
        d.nodes = d.nodes.map((n) => (ids.has(n.id) ? { ...n, group: gid } : n))
        return d
      })
    },
    ungroupSelected() {
    ...
    lockSelected(locked) {
      ...
      if (ids.size === 0) return
      commit((d) => {
        d.nodes = d.nodes.map((n) => (ids.has(n.id) ? { ...n, locked } : n))
        return d
      })
    },
    // z-index 操作: nodes 数组顺序即 z-order, 末尾在上
    bringToFront(targetIds) {
      const ids = new Set(targetIds ?? state.selectedIds)
      if (ids.size === 0) return
      commit((d) => {
        const keep = d.nodes.filter((n) => !ids.has(n.id))
        const moved = d.nodes.filter((n) => ids.has(n.id))
        d.nodes = [...keep, ...moved]
        return d
      })
    },
    sendToBack(targetIds) {
      const ids = new Set(targetIds ?? state.selectedIds)
      if (ids.size === 0) return
      commit((d) => {
        const keep = d.nodes.filter((n) => !ids.has(n.id))
    ...
    beginDrag() {
      ...
      for (const n of state.data.nodes) {
        // 锁定节点不参与拖拽
        if (ids.has(n.id) && !n.locked) startPositions[n.id] = { x: n.x, y: n.y }
      }
