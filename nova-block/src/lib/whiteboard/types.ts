...
export type FlowShape =
  ...
  | 'plus'
  | 'cloud'
  /** v0.21.14 · 表格节点 (foreignObject + HTML table) */
  | 'table'
  /** v0.21.14 · 图片节点 (嵌入 data URL) */
  | 'image'
...
/** 箭头样式 (起点/终点) */
export type ArrowStyle = 'none' | 'arrow' | 'triangle' | 'dot' | 'diamond'

/** 节点上可连接的锚点位置 */
export type Anchor = 'top' | 'right' | 'bottom' | 'left' | 'auto'

/** 笔画工具类型 (v0.21.11 新增 marker 高亮笔) */
...
export interface Stroke {
  ...
  /** v0.21.11 · 工具类型, 默认 'pen' */
  tool?: StrokeTool
  /** v0.21.11 · 透明度 (0-1); marker 默认 0.35, pen 默认 1 */
  opacity?: number
}
...
export interface FlowNode {
  ...
  color?: string
  /** v0.21.13 · 锁定; 锁定后不可拖拽/缩放/删除 */
  locked?: boolean
  /** v0.21.13 · 分组 id; 同 group 的节点一起被选中/拖拽 */
  group?: string
  /** v0.21.14 · 图片节点: base64 data URL 或 http(s) URL */
  src?: string
  /** v0.21.14 · 表格节点: 单元格二维数组 (rows[row][col]) */
  cells?: string[][]
  /** v0.21.17 · 表格列宽比例 (归一化 0~1, 长度应 = 列数); 缺省则等分 */
  colWidths?: number[]
}
